...
##			cum_gpa      => $fieldid_ref->{fieldIDcum_gpa}      || 2703,
		...
		my @vars = (
            ...
            $masked_ref->{camp_line2},
				...
				## undef, # sdec field id, nuked	## HMC comment out
				## undef, # sdec, nuked			## HMC comment out
				$masked_field_ids->{percentile},	## HMC add
				$masked_ref->{percentile},		## HMC add
				...
				## undef, ## cols 855-1274		## HMC comment out
				$masked_field_ids->{score1}, 	## HMC add
				$masked_ref->{score1},			## HMC add
				$masked_field_ids->{score2}, 	## HMC add
				$masked_ref->{score2},			## HMC add
				$masked_field_ids->{score3}, 	## HMC add
				$masked_ref->{score3},			## HMC add
				$masked_field_ids->{score5}, 	## HMC add
				$masked_ref->{score5},			## HMC add
				$masked_field_ids->{decsn_rank},	## HMC add
				$masked_ref->{decsn_rank},	## HMC add
				## undef, ## cols 939-1274	## HMC comment Date/Int ends, String starts in sWrite
				...
				$masked_ref->{res_asst},
				## type anytingundef, ## old adm_sess field id
				## undef, ## old adm_sess
				## undef, ## cols 1324-1414 ## HMC comment out
				## undef, ## old class standing field id 	- HMC comment out for adm_rec.decsn 01/26/07
				## undef, ## old class standing 			- HMC comment out for adm_rec.decsn 02/16/07
				$masked_field_ids->{decsn}, ## HMC add 01/26/07 
				$masked_ref->{decsn},       ## HMC add 01/26/07
				$masked_field_ids->{plan_enr_sess}, ## HMC add
				$masked_ref->{plan_enr_sess}, ## HMC add
				$masked_field_ids->{non_tally}, ## HMC add
				$masked_ref->{non_tally},	## HMC add
				$masked_field_ids->{resrc},	## HMC add
				$masked_ref->{resrc},		## HMC add
